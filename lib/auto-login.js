...
const { getSessionUser,logoutUser } = require('./musicoin/user');
...
console.log("autoLogin start",req.baseUrl,req.path);
    if (!req.baseUrl && canSkipPath(req.path)) {
      return next();
    }
if(req.path == '/logout'){
            console.log("logging out 1");
  logoutUser(req.headers,(error, response) => {
              req.logout();
  return next();
  });
      ...
      if (error) {
        console.log("error###########",error,error.message,req.uid)
